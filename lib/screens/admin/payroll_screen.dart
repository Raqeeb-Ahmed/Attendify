...
class _PayrollScreenState extends State<PayrollScreen> {
  ...
  Future<void> _markAsPaid(Map<String, dynamic> emp, String monthKey,
      {double deductions = 0, double advance = 0}) async {
    try {
  ...
  Future<void> _showPaymentDialog(
      BuildContext context, Map<String, dynamic> emp, String monthKey) async {
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Process Payroll – ${emp['name']}',
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (ctx, i) =>
                                  _buildEmployeePayrollCard(ctx, users[i], paidSet, paidMap),
                            ),
                          ),
                        ],
                      );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Icon(Icons.account_balance_wallet_rounded, color: Color(0xFF6366F1), size: 22),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Payroll Management',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                Text('Year: $_selectedYear', style: const TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
              ],
            ),
          ),
          Row(
            children: [
              ...
              IconButton(
                ...
                icon: Icon(Icons.chevron_right_rounded,
                    color: _selectedYear < DateTime.now().year ? const Color(0xFF6366F1) : Colors.grey.shade300),
              ),
            ],
          ),
        ],
  ...
  Widget _buildEmployeePayrollCard(BuildContext ctx, Map<String, dynamic> emp,
      Set<String> paidSet, Map<String, Map<String, dynamic>> paidMap) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                Column(
                  ...
                  children: [
                    Text('Net: PKR ${fmt.format(net)}',
                        style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1E293B))),
                    Text('Base: PKR ${fmt.format(base)} + Allow: PKR ${fmt.format(allowances)}',
            ...
            Text('Monthly Status – $_selectedYear',
            ...
            Wrap(
              ...
              children: List.generate(12, (mi) {
                ...
                final isFuture = DateTime(_selectedYear, mi + 1).isAfter(DateTime.now());
  ...
  void _showPaymentDetails(BuildContext ctx, Map<String, dynamic> data, String month) {
    final fmt = NumberFormat('#,##0', 'en_US');
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
        title: Text('Payslip – $month', style: const TextStyle(fontWeight: FontWeight.w700)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _payRow('Base Salary', 'PKR ${fmt.format(data['baseSalary'] ?? 0)}'),
            _payRow('Allowances', 'PKR ${fmt.format(data['allowances'] ?? 0)}'),
            _payRow('Tax (10%)', '– PKR ${fmt.format(data['tax'] ?? 0)}'),
            _payRow('Deductions', '– PKR ${fmt.format(data['deductions'] ?? 0)}'),
            _payRow('Advance', '– PKR ${fmt.format(data['advance'] ?? 0)}'),
            const Divider(),
            _payRow('Net Salary', 'PKR ${fmt.format(data['netSalary'] ?? 0)}', bold: true),
